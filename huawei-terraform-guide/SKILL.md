...
本 Skill 用于帮助用户通过 Terraform 创建华为云资源，但核心目标不是“直接写 Terraform 文件”，而是**先理解用户要达成的云上目标，再将其转化为可执行的 Terraform 配置**。

本 Skill 适用于以下场景：
- 用户只提出模糊目标，例如“创建一个 ECS”“给我一个 VPC”“部署一个能上网的 Linux 主机”
- 用户未提供完整参数，但 Terraform/云资源创建需要补齐关键配置
- 需要基于真实可用的 Region / AZ / Flavor / Image / Disk / Price 等信息向用户提供推荐值
- 需要在生成 Terraform 配置前与用户确认关键参数
- 默认只生成配置或执行前置查询，**禁止执行 terraform apply，除非用户明确要求 apply**
...
1. **从用户目标出发，不从 Terraform 文件出发**
   - 用户说“创建一个 ECS”，目标是获得一台云服务器，而不是几个 `.tf` 文件。
   - 只有在明确用户目标和关键参数后，才进入 Terraform 配置生成阶段。

2. **先识别意图，再补齐参数**
   - 先判断用户想创建什么资源或完成什么部署目标。
   - 若信息不完整，必须识别出缺失参数并主动补齐。

3. **推荐值必须基于真实查询结果**
   - 不得凭空编造 Region、AZ、规格、镜像、磁盘类型、价格、网络配置等默认值。
   - 推荐值必须通过可用 API、数据源、官方可验证信息或实时查询结果获得。
   - 如果无法查询到真实可用值，必须明确说明，而不是伪造默认值。

4. **询问用户时必须携带推荐值**
   - 不要只把问题抛给用户。
   - 应先查询并给出一组真实、可执行、合理的推荐值，再让用户确认或修改。
   - 如能获得价格，优先附带价格信息或价格区间。

5. **优先降低用户决策成本**
   - 对没有明确要求的参数，提供“推荐配置 + 原因 + 价格/成本信息”。
   - 让用户基于候选方案确认，而不是要求用户从零提供所有参数。

6. **默认不执行 apply**
   - 默认行为是：查询信息、整理参数、生成 Terraform 配置、说明执行方式。
   - 只有当用户明确要求时，才可以执行 `terraform apply`。
---
